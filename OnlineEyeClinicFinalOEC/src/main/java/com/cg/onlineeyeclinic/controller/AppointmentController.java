...
public class AppointmentController {
	...
	public ResponseEntity<Appointment> bookAppointment(@Valid @RequestBody Appointment appointment) {
		appointment.setConsultationFee(1000.00);
	...
	@GetMapping("/find_appointment_by_patient_id/{patientId}")
	public ResponseEntity<List<Appointment>> findAppointmentByPatientId(
			@PathVariable("patientId") Long patientId) {
		List<Appointment> appointment = appointmentService.viewAppointmentByPatientId(patientId);
		return new ResponseEntity<>(appointment, HttpStatus.OK);
	}
	
	@GetMapping("/find_appointment_by_doctor_id/{doctorId}")
	public ResponseEntity<List<Appointment>> findAppointmentByDoctorId(
			@PathVariable("doctorId") Long doctorId) {
		List<Appointment> appointment = appointmentService.viewAppointmentByDoctorId(doctorId);
		return new ResponseEntity<>(appointment, HttpStatus.OK);
	}
}
